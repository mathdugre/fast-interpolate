...
RUN : \
    ...
    && :
COPY ./exclude-metric.txt /tmp/exclude-metric.txt
# Configure ITK
...
RUN : \
    ...
    -DCMAKE_C_FLAGS="--verbose --exclude-file=/tmp/exclude-metric.txt" \
    -DCMAKE_CXX_FLAGS="--verbose --exclude-file=/tmp/exclude-metric.txt" \
    /tmp/itk/source \
    && :
# Build ITK
RUN : \
